...
class GroupSensor(SensorGroup):
    ...
    async def async_added_to_hass(self):
        ...
        self.async_on_remove(
            async_track_entity_registry_updated_event(
                self.hass, self._entity_ids, self._handle_registry_update
            )
        )
    ...
    @callback
    def _handle_registry_update(self, event):
        """Update member list when registry changes."""
        
        # We only care about label or area changes
        action = event.data.get("action")
        
        _LOGGER.info("event.data %s", event.data)
